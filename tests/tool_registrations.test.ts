 ...
 * and that registerTools wires all 23 tools to the MCP server.
 */
...
describe("TOOL_NAMES", () => {
  it("should contain 22 tool name entries", () => {
    const entries = Object.entries(TOOL_NAMES);
    assertEquals(entries.length, 22);
  });
...
describe("TOOL_DEFINITIONS", () => {
  it("should contain 22 tool definitions", () => {
    assertEquals(TOOL_DEFINITIONS.length, 22);
  });
...
describe("registerTools", () => {
  ...
  it("should register exactly 22 tools", () => {
    ...
    assertEquals(registerSpy.calls.length, 22);
    registerSpy.restore();
  });
