...
const WATERMARK_TEMPLATE =
  `<UserObject label="Generated by Draw.io MCP Server v{{VERSION}} ({{DATE}})" link="https://aka.ms/drawio-mcp-server" id="${WATERMARK_CELL_ID}">` +
  `<mxCell style="text;html=1;fontSize=11;fontColor=#999999;align=center;verticalAlign=middle;whiteSpace=wrap;overflow=hidden;strokeColor=none;fillColor=none;" vertex="1" parent="1">` +
...
export class DiagramModel {
  ...
  private renderWatermarkXml(absoluteBoundsCache: Map<string, Rect | null>): string {
    ...
    const w = 400;
    const h = 20;
    const x = Math.round(hasPositions ? (minX + maxX) / 2 - w / 2 : 850 / 2 - w / 2);
    const y = hasPositions ? maxY + 40 : 100;
