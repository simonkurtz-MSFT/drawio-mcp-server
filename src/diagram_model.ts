...
const WATERMARK_TEMPLATE =
  `<UserObject label="Generated by Draw.io MCP Server v{{VERSION}}&lt;br&gt;{{DATE}}" link="https://aka.ms/drawio-mcp-server" id="${WATERMARK_CELL_ID}">` +
  `<mxCell style="text;html=1;fontSize=11;fontColor=#000000;align=left;verticalAlign=top;whiteSpace=wrap;overflow=hidden;strokeColor=none;fillColor=none;" vertex="1" parent="1">` +
...
export class DiagramModel {
  ...
  /**
   * When an edge uses auto-computed symmetric anchors and the source/target
   * centers are not aligned, add explicit midpoint waypoints to produce a clean
   * Z-bend.  Without these the bend position is left to Draw.io's auto-router,
   * which may vary between renderers.  Skipped when the edge already carries
   * group-avoidance waypoints or explicit user anchors.
   */
  private getAlignmentWaypoints(
    edge: Cell,
    precomputedCenters: Map<string, Point>,
    absoluteBoundsCache: Map<string, Rect | null>,
  ): Point[] {
    if (edge.type !== "edge" || !edge.sourceId || !edge.targetId) {
      return [];
    }
    ...
    if (
      this.hasStyleKey(baseStyle, "exitX") || this.hasStyleKey(baseStyle, "exitY") ||
      this.hasStyleKey(baseStyle, "entryX") || this.hasStyleKey(baseStyle, "entryY")
    ) {
      return [];
    }

    const sourceCenter = precomputedCenters.get(edge.sourceId);
    const targetCenter = precomputedCenters.get(edge.targetId);
    if (!sourceCenter || !targetCenter) {
      return [];
    }
    ...
    if (isHorizontalFlow) {
      ...
      const exitX = dx >= 0 ? sourceBounds.x + sourceBounds.width : sourceBounds.x;
      const exitY = sourceCenter.y;
      const entryX = dx >= 0 ? targetBounds.x : targetBounds.x + targetBounds.width;
      const entryY = targetCenter.y;
      const midX = Math.round((exitX + entryX) / 2);
      return [
        { x: midX, y: Math.round(exitY) },
        { x: midX, y: Math.round(entryY) },
      ];
    }
    ...
    const exitX = sourceCenter.x;
    const exitY = dy >= 0 ? sourceBounds.y + sourceBounds.height : sourceBounds.y;
    const entryX = targetCenter.x;
    const entryY = dy >= 0 ? targetBounds.y : targetBounds.y + targetBounds.height;
    const midY = Math.round((exitY + entryY) / 2);
    return [
      { x: Math.round(exitX), y: midY },
      { x: Math.round(entryX), y: midY },
    ];
  }
    ...
    const cellsXml = Array.from(cells.values())
      .map((cell) => {
        if (cell.type === "vertex") {
          ...
          const avoidanceWaypoints = this.getGroupAvoidanceWaypoints(cell, precomputedCenters, precomputedGroupBounds);
          const waypoints = avoidanceWaypoints.length > 0
            ? avoidanceWaypoints
            : this.getAlignmentWaypoints(cell, precomputedCenters, absoluteBoundsCache);
